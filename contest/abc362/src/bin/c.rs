...
fn main() {
    ...
    let min_sum = LR.iter().map(|&(l, _)| l).sum::<i64>();
    let max_sum = LR.iter().map(|&(_, r)| r).sum::<i64>();
    ...
    for i in 0..N {
        ...
        if target < 0 && x[i] < r {
            let diff = min!(-target, r - x[i]);
            x[i] += diff;
            target += diff;
        } else if target > 0 && x[i] > l {
            let diff = min!(target, x[i] - l);
            x[i] -= diff;
            target -= diff;
        }
